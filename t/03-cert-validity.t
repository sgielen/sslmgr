use Test::More tests => 3;
use Test::Directory;
use Sslmgr;

my $dir = Test::Directory->new();
$dir->create("test.crt", content => <<EOF);
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOF
my ($dt_begin, $dt_end) = Sslmgr::get_cert_validity_period($dir->path, "test.crt");
is($dt_begin->datetime, "2015-03-10T00:37:16", "Begin datetime is OK");
is($dt_end->datetime, "2025-03-07T00:37:16", "End datetime is OK");
$dir->is_ok("no missing or new files");
